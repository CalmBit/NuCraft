...
public class MapGenRiftRavine extends MapGenBase
{
    ...
    {
        return data[index] == NuCraft.acidBlock;
    }
    ...
    private boolean isTopBlock(Block[] data, int index, int x, int y, int z, int chunkX, int chunkZ)
    {
        ...
        return (isExceptionBiome(biome) ? data[index] == NuCraft.riftGrass : data[index] == biome.topBlock);
    }
    ...
    {
        BiomeGenBase biome = worldObj.getBiomeGenForCoords(x + chunkX * 16, z + chunkZ * 16);
        Block top    = (isExceptionBiome(biome) ? NuCraft.riftGrass : biome.topBlock);
        Block filler = (isExceptionBiome(biome) ? NuCraft.riftDirt  : biome.fillerBlock);
        Block block  = data[index];

        if (block == NuCraft.riftStone || block == filler || block == top)
        {
            if (y < 10)
            {
                data[index] = NuCraft.riftPlasmaBlock;
            }
            else
            {
                data[index] = null;
