...
public class BlockRubberTreeSapling extends BlockBush implements IGrowable {

	protected IIcon saplingTexture;
	
	public BlockRubberTreeSapling()
	{
		super(Material.grass);
		setStepSound(soundTypeGrass);
		float f = 0.4F;
        this.setBlockBounds(0.5F - f, 0.0F, 0.5F - f, 0.5F + f, f * 2.0F, 0.5F + f);
        setBlockName("saplingRubberTree");
	}
	...
	{
		saplingTexture = register.registerIcon("nucraftworld:saplingRubberTree");
	}
	
	public void generate(World world, int x, int y, int z, Random random)
	{
		if(!net.minecraftforge.event.terraingen.TerrainGen.saplingGrowTree(world, random, x, y, z)) return;
		WorldGenRubberTrees gen = new WorldGenRubberTrees();
