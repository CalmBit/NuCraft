...
public class WorldGenElmTrees extends WorldGenerator
{
	/**
	 * Mininum tree height...
	 */
	private final static int MIN_TREE_HEIGHT = 10;
	private final static int TREE_HEIGHT_DEV = 5;
	private final static Block LEAF_BLOCK = NuCraft.leavesElm;
	private final static Block TRUNK_BLOCK = NuCraft.logElm;
	private final static int LEAF_HEIGHT = 8;
	                                   	...
	                                   	for (int tempX = x - l1; tempX <= x + l1; ++tempX)
	                                   	{
	                                       	...
	                                       	for (int tempZ = z - l1; tempZ <= z + l1; ++tempZ)
	                                       	{
	                                           	...
	                                           	if (Math.abs(j2) != l1 || Math.abs(l2) != l1 || random.nextInt(2) != 0 && i3 != 0)
	                                           	{
	                                               	...
	                                               	{
	                                               		if(tempY % 2 == 0 || tempY == y + treeHeight) world.setBlock(tempX, tempY, tempZ, LEAF_BLOCK, 0, 3);
	                                               	}
	                                           	}
	                                       	}
	                                   	}
