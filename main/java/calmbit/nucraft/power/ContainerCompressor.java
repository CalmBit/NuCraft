...
public class ContainerCompressor extends Container {
	...
	private final int MATERIAL_SLOT_POS = VANILLA_SLOT_COUNT;
	private final int FUEL_SLOT_POS = MATERIAL_SLOT_POS + 1;
	private final int RESULT_SLOT_POS = FUEL_SLOT_POS + 1;
	
	public ContainerCompressor(InventoryPlayer player, TileEntityCompressor compressor)
	{
		this.tileEntityCompressor = compressor;
		...
		addSlotToContainer(new Slot(this.tileEntityCompressor, 0, 80, 7 ));
		addSlotToContainer(new SlotFuel(this.tileEntityCompressor, 1, 39,34));
		addSlotToContainer(new SlotFurnace(player.player, this.tileEntityCompressor, 2, 81, 57));
	}
	
	@Override
	public boolean canInteractWith(EntityPlayer playerIn) {
		return tileEntityCompressor.isUseableByPlayer(playerIn);
	}
	...
	public void detectAndSendChanges()
	{
		...
		if(lastCompressTime != tileEntityCompressor.compressionTime)
		{
			lastCompressTime = tileEntityCompressor.compressionTime;
			for(int i = 0; i < this.crafters.size();i++)
			{
				ICrafting icrafting = (ICrafting)this.crafters.get(i);
				icrafting.sendProgressBarUpdate(this, 0, lastCompressTime);
			}
		}
		
		if(lastFuelTime != tileEntityCompressor.itemFuelTime)
		{
			lastFuelTime = tileEntityCompressor.itemFuelTime;
			for(int i = 0; i < this.crafters.size();i++)
			{
				ICrafting icrafting = (ICrafting)this.crafters.get(i);
		...
		if(lastFuelTotalTime != tileEntityCompressor.itemFuelTimeTotal)
		{
			lastFuelTotalTime = tileEntityCompressor.itemFuelTimeTotal;
			for(int i = 0; i < this.crafters.size();i++)
			{
				ICrafting icrafting = (ICrafting)this.crafters.get(i);
	...
	public void updateProgressBar(int id, int data)
	{
		...
		{
			case 0:
				tileEntityCompressor.compressionTime = data;
				break;
			case 1:
				tileEntityCompressor.itemFuelTime = data;
				break;
			case 2:
				tileEntityCompressor.itemFuelTimeTotal = data;
				break;
		}
	}
	...
	public class SlotFuel extends Slot
	{
		...
		{
			return TileEntityCompressor.isItemFuel(stack);
		}
