...
public class BlockCompressor extends BlockContainer {
	...
	private IIcon compressorFront;
	@SideOnly(Side.CLIENT)
	private IIcon compressorSide;
	...
	{
		compressorSide = register.registerIcon("nucraftpower:compressorSide");
		compressorFront = this.isActive ? register.registerIcon("nucraftpower:active_compressorFront") : register.registerIcon("nucraftpower:compressorFront");
	}
	...
	{
		if(metadata == 0 && side == 3) return compressorFront;
		return (side == metadata) ? compressorFront : compressorSide;
	}
	...
	public static void updateCompressorState(boolean burning, World worldIn, int bx, int by, int bz)
    {
        ...
        if (burning)
        {
            worldIn.setBlock(bx, by, bz, NuCraft.active_compressor);
        }
        else
        {
            worldIn.setBlock(bx, by, bz, NuCraft.compressor);
        }
	...
	public boolean onBlockActivated(World worldIn, int bx, int by, int bz, EntityPlayer playerIn, int side, float hitX, float hitY, float hitZ)
	{
		...
		playerIn.openGui(NuCraft.instance, GuiHandlerCompressor.getGuiId(), worldIn, bx, by, bz);
		return true;
	}
	...
	public void onBlockPlacedBy(World worldIn, int bx, int by, int bz, EntityLivingBase placer, ItemStack stack)
    {
		...
		if(stack.hasDisplayName())
		{
			...
			{
				((TileEntityCompressor)tile).setCustomInventoryName(stack.getDisplayName());
			}
		}
    }
		 ...
		 if (this.isActive)
	        {
	            ...
	            if (l == 4)
	            {
	                ...
	                worldIn.spawnParticle("cloud", (double)(f - f3), (double)f1, (double)(f2 + f4), 0.0D, 0.0D, 0.0D);
	            }
	            else if (l == 5)
	            {
	                worldIn.spawnParticle("smoke", (double)(f + f3), (double)f1, (double)(f2 + f4), 0.0D, 0.0D, 0.0D);
	            ...
	            else if (l == 2)
	            {
	                ...
	                worldIn.spawnParticle("cloud", (double)(f + f4), (double)f1, (double)(f2 - f3), 0.0D, 0.0D, 0.0D);
	            }
	            else if (l == 3)
	            {
	                worldIn.spawnParticle("smoke", (double)(f + f4), (double)f1, (double)(f2 + f3), 0.0D, 0.0D, 0.0D);
